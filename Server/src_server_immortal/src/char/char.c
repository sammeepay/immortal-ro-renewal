...
bool char_checkdb(void){
	...
	const char* sqltable[] = {
		...
		schema_config.clan_table, schema_config.clan_alliance_table, schema_config.achievement_table, schema_config.mail_attachment_db
	};
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`,`account_id`,`char_num`,`name`,`class`,"
		...
		"`moves`,`unban_time`,`font`,`sex`,`hotkey_rowshift`,`clan_id`,`title_id`,`last_login`"
