				...
				<tr class="item-drop-<?php echo $itemDrop['type'] ?>"
					...
					<td align="right">
						<?php if ($auth->actionAllowed('item_new', 'view')): ?>
							<a href="<?php echo $this->url('item_new', 'view', array('id' => $itemDrop['id'])) ?>"><?=$itemDrop['id']?></a>
						<?php else: ?>
	...
	<?php if($mobSpawn !== false) { ?>
		...
		if (sizeof($mobSpawn)) {
			...
			foreach ($mobSpawn as $spawn) { ?>
				...
				<tr>
					<td><img src="<?= mapImage($spawn->map, true) ?>"/></td>
					<?php if($auth->actionAllowed('map', 'view')){ ?>
						<td><a href="<?= $this->url('map', 'view', array('map' => $spawn->map)) ?>"><?= $spawn->map ?></td>
					<?php } else { ?>
						<td><?= $spawn->map ?></td>
					<?php } ?>
					<td><?= $spawn->count ?></td>
